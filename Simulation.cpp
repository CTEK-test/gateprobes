...
void Simulation::LayoutFromFile(std::ifstream& is)
{
	...
	std::string xml_string_begin ="<?xml"; // FIX replace regex on skipping lines
...
int Simulation::Step()
{
	...
	for (const auto& transition : transitions) // FIX avoid coping every cycle
	{
...
boost::property_tree::ptree Simulation::GetJson() const
{
	...
	for (const auto& p : m_probes) // FIX fill gates in parallel
